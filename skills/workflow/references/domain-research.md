...
| Run ID | Generated by general-purpose or `uuidgen` | Yes |
...
If gate passes: Report completion to general-purpose. The Component Manifest is the handoff artifact for the chain-composer skill.
...
**Solution**: Report to general-purpose with findings. The domain may not need decomposition — it may be a single-pipeline domain. Ask: "Only 1 subdomain found for {domain}. Proceed as single pipeline or broaden the domain scope?"
...
STOP and ask the general-purpose (wait for explicit confirmation) when:
