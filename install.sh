...
# Summary
echo ""
# INDEX.json is committed and generated by scripts/generate-*-index.py on
# purpose; the installer used to regenerate both here without
# --include-private, which could only rewrite tracked files in place (and did:
# the test suite runs install.sh and clobbered an uncommitted agents/INDEX.json
# edit on 2026-08-28). Run the generators deliberately when components change.
