...
Each agent is defined in `agents/*.md` with YAML frontmatter specifying `model`, `tools`, routing triggers, and paired skills.
...
| `react-native-engineer`             | React Native and Expo development: performance, animations, navigation, native UI patterns             |
...
| Agent                         | Description                                          |
| ----------------------------- | ---------------------------------------------------- |
| `testing-automation-engineer` | Vitest, Playwright, E2E, coverage enforcement, CI/CD |

---

## Game/Combat Rendering

| Agent                    | Description                                                                                                          |
| ------------------------ | -------------------------------------------------------------------------------------------------------------------- |
| `combat-effects-upgrade` | Zero-dependency combat visual upgrades: CSS particle replacement, Framer Motion combat juice, CSS 3D card transforms |
| `pixijs-combat-renderer` | PixiJS v8 2D WebGL combat rendering: @pixi/react hybrid canvas, normal maps, GPU particles, post-processing          |
| `rive-skeletal-animator` | Rive skeletal animation: @rive-app/react-canvas, state machines, character pipelines, combat integration             |

---

## Pipeline (PPTX/Notebook)

| Agent       | Description                                                                                |
| ----------- | ------------------------------------------------------------------------------------------ |
| `extractor` | Extract SCENARIO_EXTRACT JSON from a PPTX with confidence scores; halts on low confidence  |
| `generator` | Generate PptxGenJS notebook slides from confirmed SCENARIO_EXTRACT; runs gates G1-G9       |
| `qa-gate`   | Run QA gate checks on generated notebooks/lessons/games; blocks delivery on critical FAILs |
