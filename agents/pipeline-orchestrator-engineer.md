...
- **Domain Research First**: For domain pipeline requests, ALWAYS invoke the `workflow` skill (research phase) before composing chains. The old DISCOVER phase only checked existing components — the new Phase 1 discovers _subdomains_ within the target domain.
...
See [references/orchestration-patterns.md](pipeline-orchestrator-engineer/references/orchestration-patterns.md) (Capabilities Summary section) for the full CAN/CANNOT list. Short version: CAN orchestrate multi-component pipeline graphs with parallel fan-out; CANNOT write domain-specific business logic, modify existing pipelines, or create pipelines without routing integration.
...
**Step 1**: Create `adr/pipeline-{name}.md` using the ADR template (sections: Status, Context, Decision, Component Manifest, Constraints, Consequences, Test Plan). See [references/orchestration-patterns.md](pipeline-orchestrator-engineer/references/orchestration-patterns.md) for the full template.
...
**Planning**: Group by creator type (skill-creator for agents/skills, hook-development-engineer for Python hooks, this agent directly for scripts). See [references/orchestration-patterns.md](pipeline-orchestrator-engineer/references/orchestration-patterns.md) for the creator sub-agent table and sub-agent context package requirements.
...
**Step 5**: Verify integration (agents in INDEX.json, routing entries match triggers, hooks are valid Python, skills have frontmatter, no orphaned components). See [references/orchestration-patterns.md](pipeline-orchestrator-engineer/references/orchestration-patterns.md) for the integration verification checklist.
...
Uses the **Planning Schema** (6 required sections: Discovery Report, Pipeline Spec, Execution Plan, Integration Checklist, Completion Report, Session Restart Notice). The Session Restart Notice is MANDATORY verbatim output after every pipeline creation. Error-fix mappings (Duplicate Component, Template Validation Failure, Routing Conflict, Chain Validation Failure, Domain Research Insufficient) and Preferred Patterns (5 anti-patterns with do-instead) are in [references/anti-patterns.md](pipeline-orchestrator-engineer/references/anti-patterns.md). The Session Restart Notice verbatim text and output schema are in [references/orchestration-patterns.md](pipeline-orchestrator-engineer/references/orchestration-patterns.md).
...
| --------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------- |
| Sub-agent dispatch, fan-out, parallel scaffolding, output schema, Session Restart Notice, capabilities          | `pipeline-orchestrator-engineer/references/orchestration-patterns.md`                                  | Before any Phase 3 SCAFFOLD or when preparing sub-agent context packages              |
| Anti-pattern, duplicate component, skipping discovery, routing conflict, error-fix mappings, preferred patterns | `pipeline-orchestrator-engineer/references/anti-patterns.md`                                           | Before Phase 1 DISCOVER, before Phase 4 INTEGRATE, when reviewing pipeline for issues |
| Error from `validate-chain`, `audit-tool-restrictions`, `adr-query`                                             | `pipeline-orchestrator-engineer/references/anti-patterns.md` (Error-Fix Mappings section)              | When any of these scripts returns an error                                            |
| Gate enforcement, phase transition, fan-in collection                                                           | `pipeline-orchestrator-engineer/references/orchestration-patterns.md` (Phase Gate Enforcement section) | Before transitioning between any two phases                                           |

## References

For detailed information:

- **Orchestration Patterns**: [references/orchestration-patterns.md](pipeline-orchestrator-engineer/references/orchestration-patterns.md)
- **Anti-Patterns**: [references/anti-patterns.md](pipeline-orchestrator-engineer/references/anti-patterns.md)
